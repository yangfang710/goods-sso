...
public class UserController {
	...
	public Object checkData(@PathVariable String param, @PathVariable Integer type, String callback) {
		...
		if (StringUtils.isBlank(param)) {
			result = GoodsResult.build(400, "校验内容不能为空");
		}
		if (type == null) {
			result = GoodsResult.build(400, "校验内容类型不能为空");
		}
		if (type != 1 && type != 2 && type != 3) {
			result = GoodsResult.build(400, "校验内容类型错误");
		}
		// 校验出错
		if (null != result) {
			if (null != callback) {
		...
		try {
			...
			result = GoodsResult.build(500, ExceptionUtil.getStackTrace(e));
		}
	...
	public GoodsResult createUser(TbUser user) {
		...
		try {
			GoodsResult result = userService.createUser(user);
			return result;
		} catch (Exception e) {
	...
	public GoodsResult userLogin(String username, String password, HttpServletRequest request,
		...
		try {
			GoodsResult result = userService.userLogin(username, password, request, response);
			return result;
		} catch (Exception e) {
			e.printStackTrace();
			return GoodsResult.build(500, ExceptionUtil.getStackTrace(e));
